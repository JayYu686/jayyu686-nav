...
- **低配设备自动降级**：首次加载时根据 `navigator.deviceMemory` 与 `navigator.hardwareConcurrency` 自动识别低配设备；若用户未手动设置偏好，默认启用 `no-effects`，减少高成本动效。
- **搜索热路径优化**：为卡片建立预计算搜索索引（标题、别名、描述），输入改为防抖执行，减少连续输入时的重复字符串处理与 DOM 读取。
- **筛选与排序降耗**：仅在存在搜索词时按评分重排卡片；排序后仅在顺序真实变化时才触发 DOM 重新插入，降低重排与重绘频率。
- **观察器刷新节流**：分类筛选后的 `IntersectionObserver` 目标刷新改为 `requestAnimationFrame` 调度，避免同步频繁 `observe/unobserve` 导致主线程抖动。
- **最近访问模块减负**：最近访问图标从第三方 favicon 请求改为本地生成 SVG data URL，并使用 `DocumentFragment` 批量插入节点，减少网络依赖与渲染开销。
- **验收结果**：执行 `npm run check`（`astro check` + `astro build`）通过；无新增错误，构建成功。
